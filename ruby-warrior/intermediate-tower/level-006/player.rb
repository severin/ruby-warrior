class Player
  ...
  def play_turn(warrior)
    ...
    if !enemy_position(warrior) && warrior.health < 14
      ...
      if warrior.feel(captive).stairs? && (retreat = retreat_position(warrior))
        warrior.walk! retreat
      else
  ...
  def distant_captive_position(warrior)
    if ((sp = warrior.listen.select {|sp| sp.ticking?}.first) || (sp = warrior.listen.select {|sp| sp.ticking?}.first)) && !captive_position(warrior)
